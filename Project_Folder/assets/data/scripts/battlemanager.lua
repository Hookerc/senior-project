-- Loading all of the graphic files used in the battle scene
...
attack_icon = love.graphics.newImage("assets/graphics/system/battle_attack.png")
defend_icon = love.graphics.newImage("assets/graphics/system/battle_defend.png")
skill_icon = love.graphics.newImage("assets/graphics/system/battle_skills.png")
item_icon = love.graphics.newImage("assets/graphics/system/battle_items.png")
select_icon = love.graphics.newImage("assets/graphics/system/battle_select.png")

--Loading all of the audio files used in the battle scene
victory_theme = love.audio.newSource("assets/audio/bgm/victory.ogg")
battle_se_confirm = love.audio.newSource("assets/audio/se/se_confirm.wav")
battle_se_cancel = love.audio.newSource("assets/audio/se/se_cancel.wav")
...
--Resets all battle parameters to default
--takes a list of enemies generated by battler:new() and an optional bgm
function battle_manager:new_battle(enemy_party, new_bgm)
	...
	self.message_log = ""
	self.animate_time = 0
	local tempbgm = new_bgm or "battle.ogg"
	self.bgm = love.audio.newSource(audio_source .. "bgm/" .. tempbgm, "stream")
end

function battle_manager:set_battleback(background) -- loads the background image to be used in battle
	self.battleback = background
end

function battle_manager:update(dt) --calls the proper function for the phase of the battle
...
function battle_manager:phase_animate(dt) --displays the animation until animate_time runs out
...
function battle_manager:phase_battle_init(dt) --sets up the battle, creating battle turn order.
	self:create_turn_order()
	self.phase = "turn"
	self.current_menu = "menu"
end

-- Determies if the current actor is a player or enemy and calls the proper update
-- then checks if the turn is over, and moves to the next actor.
function battle_manager:phase_turn(dt)
	local isPlayer = self:playerturn(self.turn_order[self.current_actor])
	if isPlayer then
		if self.player.player_party[self.current_actor].hp < 1 then
			self.turn_over = true
		else
...
-- Checks for enemy/player death and victory/defeat conditions
...
function battle_manager:player_turn(actor)
	if self.current_menu == "menu" then
		...
		if self.keypressed == "z" then
			...
			if self.index == 4 and #self.player.inventory > 0 then
	...
	if self.current_menu == "item" then
		...
		if self.keypressed == "z" then
			--do skill stuff
				self.selected_action = self.player.inventory[self.index2]
				self.targeting = self.selected_action.target_type
				self.index3 = 1
				self.keypressed = ""
				self.current_menu = "target"
				self.previous_menu = "item"
		end
		if self.keypressed == "x" then
			self.current_menu = self.previous_menu
			self.index2 = 1
...
function battle_manager:enemy_turn(actor)
	...
	for i,k in ipairs(self.player.player_party) do
		if k.hp > 0 then table.insert(available_targets, k) end
	end
	
	local rand_target = love.math.random(1, #available_targets)
	...
	--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	self.message_log = used_skill:use(actor, available_targets[rand_target]) .. " Skill: " .. rand_skill .. " out of " .. #actor.skills
	self.animate_time = 2
	self.phase = "animate"
	self.turn_over = true
end
...
function battle_manager:phase_victory(dt)
	self.animate_time = self.animate_time - dt
	if self.animate_time > 0 then self.keypressed = "" end
...
function battle_manager:draw()
	...
	if self.phase == "turn" then
		if self:playerturn(self.turn_order[self.current_actor]) ~= false then
			if self.current_menu == "menu" then self:draw_menu(self.current_actor) end
			if self.current_menu == "skill" then
...
function battle_manager:draw_info()
	...
	--portrait
	for i, k in ipairs(self.player.player_party) do
		love.graphics.draw(menu_graphic2, 200 * (i - 1), love.graphics:getHeight() - k.battle_image:getHeight())
		love.graphics.draw(k.battle_image, 200 * (i - 1), love.graphics:getHeight() - k.battle_image:getHeight())
		love.graphics.draw(menu_graphic1, 200 * (i - 1), love.graphics:getHeight() - k.battle_image:getHeight())
	end
	--information
	for i, k in ipairs(self.player.player_party) do
		love.graphics.print(tostring(k.name), (200 * (i-1)) + k.battle_image:getWidth(), love.graphics:getHeight() - k.battle_image:getHeight())
		...
		love.graphics.print( "MP:"..tostring(k.mp) .. "/" .. tostring(k.stats.maxmp), (200 * (i-1)) + k.battle_image:getWidth(), love.graphics:getHeight() - k.battle_image:getHeight() + 40)
	end
...
function battle_manager:draw_menu()
	...
	love.graphics.draw(select_icon, mx+20+30, my+((self.index-1) * 30))
	
	love.graphics.draw(attack_icon, mx+20+30, my)
	love.graphics.draw(defend_icon, mx+20+30, my+30)
	if #self.player.player_party[self.current_actor].skills == 0 then love.graphics.setShader(monoChromeShader) end
	love.graphics.draw(skill_icon, mx+20+30, my+60)
	love.graphics.setShader()
	if #self.player.inventory == 0 then love.graphics.setShader(monoChromeShader) end
	love.graphics.draw(item_icon, mx+20+30, my+90)
	love.graphics.setShader()
end
...
function battle_manager:draw_menu2(menu_type)
	...
	if menu_type == "item" then
		if self.player.inventory[self.index2 - 1] ~= nil then love.graphics.print("  " ..self.player.inventory[self.index2 - 1].name, mx + 100, my + 50) end
